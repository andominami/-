/**
 * セミナー資料ライブラリの「再生数」を数えるための、単体のGoogle Apps Scriptプロジェクト。
 * Webアプリとして公開し、サイト側(assets/app.js)からGET通信で叩く。
 *
 * セットアップ手順は automation/view-counter-README.md を参照。
 *
 * エンドポイント(すべてGETのみ):
 *   ?action=counts       … 全資料の再生数を { "form-xxxx": 3, "form-yyyy": 10, ... } の形で返す
 *   ?action=hit&id=xxx    … 指定した資料IDの再生数を1増やし、増えた後の件数を返す
 *
 * 保存先はこのスクリプト自体のプロパティ(PropertiesService)。
 * スプレッドシートやGitHub・Driveへのアクセス権限は一切使わない、独立した小さな仕組み。
 * フォーム連携の google-form-to-github.gs とは完全に別のプロジェクトとして動かす。
 */
...
/** 再生数を1増やす。同時アクセスでのカウントロスを防ぐためロックを取る。 */
